...
public class CommodityController extends ApiController {
    ...
    public R searchOrder(@RequestBody Map<String, String[]> map) {
        int page = 1;
        int status = 1;
        if (mapExist(map, "status"))
            status = Integer.parseInt(map.get("status")[0]);
        QueryWrapper<Commodity> wrapper = new QueryWrapper<Commodity>().eq("status", status);

        if (mapExist(map, "page"))
            page = Integer.parseInt(map.get("page")[0]);

        if (mapExist(map, "shop_id"))
            wrapper.eq("shop_id", map.get("shop_id")[0]); //按店铺

        if (mapExist(map, "kind_id"))
            wrapper.eq("kind_id", map.get("kind_id")[0]); //按类别
        ...
        if (mapExist(map, "asc"))
            wrapper.orderByAsc(map.get("asc")); //关键列升序

        if (mapExist(map, "desc"))
            wrapper.orderByDesc(map.get("desc")); //关键列降序
        return success(this.commodityService.page(new Page<>(page, 15), wrapper)); //返回列表
    }
